...
class FaceDetector {
    ...
    fileprivate func drawOnImage(source: UIImage,
        ...
        for faceLandmarkRegion in faceLandmarkRegions {
            ...
            let mappedPoints = points.map { CGPoint(x: boundingRect.origin.x * source.size.width + $0.x * rectWidth, y: source.size.height - (boundingRect.origin.y * source.size.height + $0.y * rectHeight)) }
            ...
            if item == 1 || item == 2 {
                context.closePath()
                if item == 1 {
                    leftCGPoints = mappedPoints
                    print(mappedPoints)
                    self.leftImage = ZImageCropper.cropImage(ofImage:source, withinPoints: leftCGPoints)
                    print("Adas")
                }
                if item == 2 {
                    leftCGPoints = mappedPoints
                    print(mappedPoints)
                    self.rightImage = ZImageCropper.cropImage(ofImage:source, withinPoints: leftCGPoints)
                    print("Adas")
